...
export type ThemeMode = 'dark' | 'light' | 'emerald' | 'cyber';
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [theme, setThemeState] = useState<ThemeMode>(() => {
    ...
    if (saved && ['dark', 'light', 'emerald', 'cyber'].includes(saved)) {
      return saved;
    }
  ...
  useEffect(() => {
    ...
    root.classList.remove('dark', 'light', 'theme-light', 'theme-emerald', 'theme-cyber');
  ...
  const toggleDarkMode = () => {
    setThemeState(prev => (prev === 'dark' || prev === 'emerald' || prev === 'cyber' ? 'light' : 'dark'));
  };
