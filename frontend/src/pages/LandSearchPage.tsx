...
import { Search, MapPin, Filter, ArrowRight, ShieldCheck, Radio, CheckCircle2, Copy, Check, Sparkles } from 'lucide-react';
...
export const LandSearchPage: React.FC<LandSearchPageProps> = ({ lang, onShowToast }) => {
  ...
  const applyPresetFilter = (preset: 'BOKARO_DEMO' | 'KHATA_125' | 'CLEAR') => {
    if (preset === 'BOKARO_DEMO') {
      setSelectedDistrict('Bokaro');
      setSelectedAnchal('Chas');
      setSelectedMauza('Kura');
      setKhataNo('125');
      setKhesraNo('450/2');
      setOwnerName('');
      setQueryText('');
    } else if (preset === 'KHATA_125') {
      setSelectedDistrict('Bokaro');
      setSelectedAnchal('Chas');
      setSelectedMauza('');
      setKhataNo('125');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-md space-y-4">
        ...
        <div className="flex items-center space-x-2 text-xs overflow-x-auto pb-1">
          ...
          <button
            type="button"
            onClick={() => applyPresetFilter('BOKARO_DEMO')}
          ...
          >
            🎯 Bokaro Chas Demo Parcel (K450/2)
          </button>
          <button
            type="button"
            onClick={() => applyPresetFilter('KHATA_125')}
      ...
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-500 text-xs space-y-2">
            <div>No matching land parcels found.</div>
            <button
              onClick={() => applyPresetFilter('BOKARO_DEMO')}
              className="text-sky-600 dark:text-sky-400 font-bold hover:underline"
            >
              Try Bokaro Demo Parcel Search →
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
